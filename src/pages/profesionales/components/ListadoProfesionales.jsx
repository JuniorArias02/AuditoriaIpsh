		...
		<div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
			...
			<div ref={parentRef} className="overflow-auto" style={{ height: "600px" }}>
				...
				<div className="bg-gray-50 sticky top-0 z-10 flex">
					<div className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-5/12">Profesional</div>
					<div className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/12">Cédula</div>
					<div className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-2/12">Cargo</div>
					<div className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/12">Estado</div>
					<div className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-2/12">Fecha Creación</div>
					<div className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-1/12">Acciones</div>
				</div>
					...
					{rowVirtualizer.getVirtualItems().map(virtualRow => {
						...
						return (
							...
							<div
								...
								// Clase 'flex' es crucial para que los hijos se alineen como columnas
								className="hover:bg-gray-50 border-b flex items-center"
							>
								...
								<div className="px-6 py-4 w-5/12">
									<div className="flex items-center">
										...
										<div className="ml-4 truncate">
											<div className="text-sm font-medium text-gray-900 truncate">{profesional.nombre}</div>
											<div className="text-sm text-gray-500">ID: {profesional.id}</div>
										</div>
									</div>
								</div>
								<div className="px-6 py-4 text-sm text-gray-900 w-1/12">
								...
								<div className="px-6 py-4 w-2/12">
									<span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getCargoBadgeColor(profesional.cargo)}`}>
								...
								<div className="px-6 py-4 w-1/12">
									{profesional.activo ? (
										<div className="flex items-center gap-2">
											...
											<span className="text-sm text-green-700 whitespace-nowrap">Activo</span>
										</div>
									) : (
										<div className="flex items-center gap-2">
											<XCircle className="w-4 h-4 text-red-500" />
											<span className="text-sm text-red-700 whitespace-nowrap">Inactivo</span>
										</div>
									)}
								</div>
								<div className="px-6 py-4 text-sm text-gray-900 whitespace-nowrap w-2/12">{profesional.created_at ? new Date(profesional.created_at).toLocaleDateString("es-ES") : "N/A"}</div>
								<div className="px-6 py-4 flex items-center gap-2 w-1/12">
									<button
										onClick={() => onEditarProfesional(profesional)} // 👈 acá el cambio
										className="text-[#2068A6] hover:text-[#1a568c] p-1 rounded"
									>
										<Edit className="w-4 h-4" />
									</button>
									<button className="text-gray-400 hover:text-gray-600 p-1 rounded"><MoreVertical className="w-4 h-4" /></button>
								</div>
							</div>
						);
					})}
			...
			<div className="px-6 py-4 border-t border-gray-200 bg-gray-50 text-sm text-gray-600">
				Mostrando <span className="font-medium">{filteredProfesionales.length}</span> de <span className="font-medium">{profesionales.length}</span> profesionales
			</div>
